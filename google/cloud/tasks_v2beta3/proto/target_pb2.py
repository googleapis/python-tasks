...
HttpRequest = _reflection.GeneratedProtocolMessageType(
  ...
  Attributes:
      ...
      headers:
          ...
          For example, ``Content-Type`` can be set to ``"application
          /octet-stream"`` or ``"application/json"``.  Headers which can
          have multiple values (according to RFC2616) can be specified
          using comma-separated values.  The size of the headers must be
          less than 80KB.
      body:
...
AppEngineHttpRequest = _reflection.GeneratedProtocolMessageType(
  ...
  [deadline][google.cloud.tasks.v2beta3.Task.dispatch_deadline]. Failed
  ...
  Attributes:
      http_method:
          ...
          /creating-handlers#writing_a_push_task_request_handler>`_ and
          the documentation for the request handlers in the language
          your app is written in e.g. `Python Request Handler <https://c
          loud.google.com/appengine/docs/python/tools/webapp/requesthand
          lerclass>`_.
      app_engine_routing:
...
OAuthToken = _reflection.GeneratedProtocolMessageType(
  ...
  Attributes:
      service_account_email:
          \ `Service account email <https://cloud.google.com/iam/docs
          /service-accounts>`_ to be used for generating OAuth token.
          The service account must be within the same project as the
          queue. The caller must have iam.serviceAccounts.actAs
...
OidcToken = _reflection.GeneratedProtocolMessageType(
  ...
  Attributes:
      service_account_email:
          \ `Service account email <https://cloud.google.com/iam/docs
          /service-accounts>`_ to be used for generating OIDC token.
          The service account must be within the same project as the
          queue. The caller must have iam.serviceAccounts.actAs
          permission for the service account.
      audience:
